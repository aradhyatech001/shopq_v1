...
// Storage files — robust fallback image server. Works on `php artisan serve`,
// Apache and Nginx without relying on the public/storage symlink or .htaccess,
// and inherits CORS from the global HandleCors middleware (matches api/*).
Route::get('/files/{path}', function (string $path) {
    $baseDir  = realpath(storage_path('app/public'));
    $fullPath = realpath($baseDir . DIRECTORY_SEPARATOR . $path);

    // Path-traversal guard: the resolved path must exist AND stay inside the
    // public storage directory. Rejects requests like /api/files/../../.env.
    if ($baseDir === false || $fullPath === false || !str_starts_with($fullPath, $baseDir)) {
        return response()->json(['error' => 'File not found'], 404);
    }

    // Images are content-addressed (uniqid-prefixed), so they never change once
    // written — cache them aggressively.
