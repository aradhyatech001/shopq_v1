  ...
  Future<void> login() async {
    ...
    try {
      ...
      if (data["status"] == "success") {
        ...
        // Store user_id & name to avoid extra API calls on every screen
        if (data['user'] != null) {
        ...
        if (!mounted) return;
        // Skip the pincode screen if one is already stored on this device.
        final storedPincode = prefs.getString('pincode_code') ?? prefs.getString('pincode_id');
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => (storedPincode == null || storedPincode.isEmpty)
                ? LocationScreen()
