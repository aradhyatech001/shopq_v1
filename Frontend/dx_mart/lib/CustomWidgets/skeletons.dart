...
///
/// Drop a composed skeleton straight into a screen's loading branch — each one
/// wraps itself in a single [Shimmer], so the whole group pulses together. Use
/// [SkeletonBox] as the grey building block when composing a custom skeleton.
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white, // tint comes from the Shimmer wrapper
...
/// A single product card placeholder (image + 2 lines + price/button row).
///
/// The image flexes ([Expanded]) so the card always fits the height it is given
/// — no vertical overflow regardless of the grid's aspect ratio. Pass [width]
/// for a fixed-width card (horizontal lists); leave it null to fill the parent
/// cell (grids). The parent must give a bounded height (grid cell or SizedBox).
...
/// A horizontally scrolling row of product placeholders (matches a home/cart
...
/// A grid of product placeholders (category view / search results).
...
/// A grid of circular category placeholders + labels.
...
/// A full home-screen loading state: banner + category row + product sections.
...
/// A list of order-card placeholders (My Orders).
...
/// Generic list-tile placeholders (wishlist, addresses, simple lists).
...
/// Product-details loading state: gallery + title + price + chips + button.
