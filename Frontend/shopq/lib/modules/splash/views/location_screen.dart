...
class _LocationScreenState extends State<LocationScreen> {
  ...
  Future<void> _confirmLocation() async {
    ...
    try {
      if (AppStorage.token.isNotEmpty) {
        await ApiHelper.post(
          ApiConstants.SET_PINCODE,
          body: {'pincode_id': _pincodeData!['id'].toString()},
          auth: true,
        );
      }

      AppStorage.pincodeId       = int.tryParse(_pincodeData!['id'].toString()) ?? 0;
      AppStorage.pincodeCode     = _pincodeData!['code'].toString();
      AppStorage.pincodeAreaName = _pincodeData!['area_name']?.toString() ?? '';
      AppStorage.pincodeCity     = _pincodeData!['city']?.toString() ?? '';
      AppStorage.pincodeState    = _pincodeData!['state']?.toString() ?? '';
